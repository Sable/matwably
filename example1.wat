...
;; Array Operations
   ...
   (export "power_SM" (func $power_SM))
	...
	(func $example1_S (param $e_f64 f64)(result i32) ;; Function signature using value analysis
		(local $k_f64 f64)(local $d_i32 i32)(local $f_f64 f64)(local $j_f64 f64)(local $e_i32 i32)
		(local $k_i32 i32)(local $f_i32 i32)(local $v_f64 f64)(local $i_f64 f64)(local $mc_t7_f64 f64)
		(local $mc_t34_i32 i32) ;; Set of locals collected in the program.

		;; Simple assignment to scalar
		f64.const 1.0
		set_local $i_f64
		f64.const 2.0
		set_local $j_f64
		f64.const 10.0
		set_local $k_f64
		loop $loop_mc_t19 ;; Labeled loops for continue;
		    ...
			;; Loop initialization
			block $block_mc_t20 ;; Labeled block for breaks
				get_local $v_f64
				get_local $k_f64
				f64.gt
				br_if $block_mc_t20 ;; end of loop condition
				f64.const 3.0
				set_local $mc_t8_f64
				get_local $v_f64
				get_local $mc_t8_f64
				...
				get_local $v_f64
				call $disp_S ;; Specialized call to disp for scalar
				br $loop_mc_t19 ;; Back to beginning of loop
			end

		end

		... ;; Parameter set up for ones.
		call $ones
		set_local $d_i32
		...
		call $horzcat ;; To make call consistent we must box scalar input arguments
		set_local $e_i32

        ;; Scalar operation simplified to a few instructions
		get_local $j_f64
		get_local $k_f64
		f64.add
		set_local $f_f64

        ;; Call to specialized plus_SM function.
		get_local $i_f64
		get_local $e_i32
		call $plus_SM
        set_local $f_i32

        ;; Call to specialized plus_MM function.
	    get_local $e_i32
	    get_local $e_i32
		call $plus_MM
		set_local $f_i32
		...
		;; ... set up parameters
		call $eye
		set_local $k_i32 ;; Since no conflict, creation of $k_i32 $k_f64 local variables

        ;; Return of an array and a scalar, scalar must be boxed.
		i32.const 2
		call $create_mxvector
		...
		i32.const 1
		get_local $i_f64
		call $convert_scalar_to_mxarray ;; Boxing scalar for return
		call $set_array_index_i32_no_check

		get_local $mc_t34_i32 ;; Load the return array value onto stack, contains [d,i]
	)
	(export "example1_S" (func $example1_S ) )
)
