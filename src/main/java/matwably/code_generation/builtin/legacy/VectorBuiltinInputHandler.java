...
//        result.setTargetName(input_arg);
        ...
        result.addInstructions(MachArrayIndexing.createCellVector(arguments.size()));
        ...
        for (ast.Expr expr :arguments) {
            if(expr instanceof NameExpr){
                ...
                if( bmv.getShape().isScalar()) {
                    ...
                    result.addInstructions(MachArrayIndexing.setArrayIndexI32NoCheck(input_arg,i-1,new GetLocal(new Idx(typedNewName))));
                }else{
                    String typedName = Util.getTypedLocalI32(name);
                    result.addInstructions(MachArrayIndexing.setArrayIndexI32NoCheck(input_arg,i-1,new GetLocal(new Idx(typedName))));
