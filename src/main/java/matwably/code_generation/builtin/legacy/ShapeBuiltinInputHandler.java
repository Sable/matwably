...
//        result.setTargetName(input_arg);
        ...
        if(sizeArgs == 0){
            result.addInstructions(MachArrayIndexing.createF64Vector(2));
            result.addInstruction(new SetLocal(new Idx(input_arg)));
            result.addInstructions(MachArrayIndexing.setArrayIndexF64NoCheck(input_arg, 0, new ConstLiteral(new F64(), 1)));
            result.addInstructions(MachArrayIndexing.setArrayIndexF64NoCheck(input_arg, 1, new ConstLiteral(new F64(), 1)));
        }else if(sizeArgs == 1){
            NameExpr nameExpr = (NameExpr)arguments.getChild(0);
            BasicMatrixValue bmv = Util.getBasicMatrixValue(valueAnalysis,(ast.ASTNode)stmt,
                    nameExpr.getName().getID());
            if(bmv.hasShape() && bmv.getShape().isScalar()){
                ...
                result.addInstructions(MachArrayIndexing.setArrayIndexF64NoCheck(input_arg, 0, new GetLocal(new Idx(expr1))));
                result.addInstructions(MachArrayIndexing.setArrayIndexF64NoCheck(input_arg, 1, new GetLocal(new Idx(expr1))));
            }else if(bmv.hasShape()&&!bmv.getShape().isRowVector()) {
                throw new Error("Size vector should be a row vector with real elements.");
            }
        }else{
            result.addInstructions(MachArrayIndexing.createF64Vector(sizeArgs));
            result.addInstruction(new SetLocal(new Idx(input_arg)));
            ...
            for(ast.Expr argExpr: arguments){
                ...
                result.addInstructions(MachArrayIndexing.setArrayIndexF64NoCheck(input_arg, i,
