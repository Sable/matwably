...
public class Cat extends AbstractConcat {
    ...
    private boolean generateDimensionCode(){
        if(arguments.size() >= 1){
            boolean returnValue = this.valueUtil.isScalar(arguments.getNameExpresion(0),node);
            // Check if argument is scalar, if it is generate, if not, we assume it is either a boxed scalar or an input vec
            if(returnValue){
                this.result.addInstructions(this.nameExpressionGenerator.genNameExpr(arguments.getNameExpresion(0),node));
            }else{
                this.result.addInstructions(this.nameExpressionGenerator.genNameExpr(arguments.getNameExpresion(0),node));
            }
            return returnValue;
     ...
     * The arrays are generated by using the VectorInputGenerator {@see  matwably.code_generation.builtin.trial.VectorInputGenerator}
     */
    @Override
    public void generateExpression() {
        if(arguments.size() == 0){
            throw new MatWablyError.NotEnoughInputArguments(callName, (ASTNode) node);
            ...
            if(arguments.size() == 1){
                if(dimIsScalar){
                    ...
                    this.result.addInstruction(new Call(new Idx("cat_M")));
                }
            }else{
                TIRCommaSeparatedList list = (TIRCommaSeparatedList) arguments.copy();
                list.removeChild(0);// Remove dimension, and generate the rest
                VectorInputGenerator vectorInputGenerator = new VectorInputGenerator(node,list, targets,matwably_analysis_set,result);
                vectorInputGenerator.generate();
                if(dimIsScalar){
                    this.result.addInstruction(new Call(new Idx("cat_SM")));
                }else{
                    this.result.addInstruction(new Call(new Idx("cat_MM")));
                }
            }
        }
    }
