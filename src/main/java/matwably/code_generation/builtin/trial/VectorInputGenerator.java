...
public class VectorInputGenerator extends DefaultBuiltinGenerator {
     ...
     */
    public VectorInputGenerator(TIRNode node, TIRCommaSeparatedList arguments, TIRCommaSeparatedList targs, String callName, IntraproceduralValueAnalysis<AggrValue<BasicMatrixValue>> analysis, InterproceduralFunctionQuery functionQuery, NameExpressionGenerator nameExpressionGenerator) {
...
//        result.setTargetName(input_arg);
        ...
        for (ast.Expr expr :arguments) {
            if(expr instanceof NameExpr){
                ...
                BasicMatrixValue bmv = Util.getBasicMatrixValue(functionAnalysis, node, name);
                ...
                String arrayName = (node instanceof TIRArrayGetStmt)?
                        ((TIRArrayGetStmt) node).getArrayName().getID():
                        ((TIRArraySetStmt) node).getArrayName().getID();
                BasicMatrixValue bmv = Util.getBasicMatrixValue(functionAnalysis, node,arrayName);
                if(!bmv.hasShape()){
