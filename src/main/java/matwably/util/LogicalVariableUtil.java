...
/**
 * A logical variable in Matwably is one that is only used in a logical context.
 * A logical context is either a while stmt or an if stmt.
 */
public class LogicalVariableUtil  {
    ...
    public boolean isUseLogical(Name name) {
        ...
        return (useStmt instanceof TIRIfStmt || useStmt instanceof TIRWhileStmt) && (defs.size() > 0 &&
                defs.stream().allMatch((Def def)-> def instanceof TIRCallStmt && isCallLogical((TIRCallStmt) def)));
    }
